...
public class DifferentialDrive {
    ...
    // This is "equivalent" in that it also absorbs the effects of drivetrain
    // inertia.
    // Measure by doing drivetrain acceleration characterization while turning in
    // place.
    protected final double moi_;

    // Drag torque (proportional to angular velocity) that resists turning, in
    // N*m/rad/s
    // Empirical testing of our drivebase showed that there was an unexplained loss
    // in torque ~proportional to angular
    // velocity, likely due to scrub of wheels.
    // NOTE: this may not be a purely linear term, and we have done limited testing,
    // but this factor helps our model to
    // better match reality. For future seasons, we should investigate what's going
    // on here...
    protected final double angular_drag_;

    // Self-explanatory. Measure by rolling the robot a known distance and counting
    // encoder ticks.
    protected final double wheel_radius_; // m

    // "Effective" kinematic wheelbase radius. Might be larger than theoretical to
    // compensate for skid steer. Measure
    // by turning the robot in place several times and figuring out what the
    ...
    public void solveForwardDynamics(DriveDynamics dynamics) {
        final boolean left_stationary = Util.epsilonEquals(dynamics.wheel_velocity.left, 0.0)
                && Math.abs(dynamics.voltage.left) < left_transmission_.friction_voltage();
        final boolean right_stationary = Util.epsilonEquals(dynamics.wheel_velocity.right, 0.0)
        ...
        dynamics.wheel_torque.left = left_transmission_.getTorqueForVoltage(dynamics.wheel_velocity.left,
                dynamics.voltage.left);
        dynamics.wheel_torque.right = right_transmission_.getTorqueForVoltage(dynamics.wheel_velocity.right,
        ...
        dynamics.chassis_acceleration.angular = effective_wheelbase_radius_
                * (dynamics.wheel_torque.right - dynamics.wheel_torque.left) / (wheel_radius_ * moi_)
        ...
        dynamics.wheel_acceleration.left = dynamics.chassis_acceleration.linear
                - dynamics.chassis_acceleration.angular * effective_wheelbase_radius_;
        dynamics.wheel_acceleration.right = dynamics.chassis_acceleration.linear
                + dynamics.chassis_acceleration.angular * effective_wheelbase_radius_;
    }
    ...
    // Assumptions about dynamics: velocities and accelerations provided, curvature
    // and dcurvature computed.
    public void solveInverseDynamics(DriveDynamics dynamics) {
        // Determine the necessary torques on the left and right wheels to produce the
        ...
        dynamics.voltage.left = left_transmission_.getVoltageForTorque(dynamics.wheel_velocity.left,
                dynamics.wheel_torque.left);
        dynamics.voltage.right = right_transmission_.getVoltageForTorque(dynamics.wheel_velocity.right,
    ...
    // angular. It is the responsibility of
    // the caller to ensure that curvature = angular vel / linear vel in these
    // cases.
    public MinMax getMinMaxAcceleration(final ChassisState chassis_velocity, double curvature,
        ...
        for (boolean left : Arrays.asList(false, true)) {
            for (double sign : Arrays.asList(1.0, -1.0)) {
                ...
                // NOTE: variable_torque is wrong. Units don't work out correctly. We made a
                // math error somewhere...
                // Leaving this "as is" for code release so as not to be disingenuous, but this
                // whole function needs
                // revisiting in the future...
                if (left) {
                    variable_torque = ((/*-moi_ * chassis_velocity.linear * chassis_velocity.linear * dcurvature*/ -drag_torque)
                            * mass_ * wheel_radius_
                            + fixed_torque *

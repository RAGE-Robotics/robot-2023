...
public class SplineGenerator {
     ...
     */
    public static List<TrajectoryPoint<Pose2dWithCurvature, Rotation2d>> parameterizeSpline(Spline s,
            List<? extends Rotation2d> headings, double maxDx, double maxDy, double maxDTheta, double t0, double t1) {
    ...
    public static List<TrajectoryPoint<Pose2dWithCurvature, Rotation2d>> parameterizeSpline(Spline s,
            List<? extends Rotation2d> headings, double maxDx, double maxDy, double maxDTheta) {
        return parameterizeSpline(s, headings, maxDx, maxDy, maxDTheta, 0.0, 1.0);
    }

    public static List<TrajectoryPoint<Pose2dWithCurvature, Rotation2d>> parameterizeSplines(List<Spline> splines,
            List<? extends Rotation2d> headings) {
        return parameterizeSplines(splines, headings, kMaxDX, kMaxDY, kMaxDTheta);
    }

    public static List<TrajectoryPoint<Pose2dWithCurvature, Rotation2d>> parameterizeSplines(
            List<? extends Spline> splines, List<? extends Rotation2d> headings, double maxDx, double maxDy,
            double maxDTheta) {
        List<TrajectoryPoint<Pose2dWithCurvature, Rotation2d>> rv = new ArrayList<>();
        if (splines.isEmpty())
            return rv;
        ...
        for (int i = 0; i < splines.size(); i++) {
            ...
            List<TrajectoryPoint<Pose2dWithCurvature, Rotation2d>> samples = parameterizeSpline(s, spline_rots, maxDx,
                    maxDy, maxDTheta);
            samples.remove(0);
            rv.addAll(samples);
        }
    ...
    private static void getSegmentArc(Spline s, List<? extends Rotation2d> headings,
            List<TrajectoryPoint<Pose2dWithCurvature, Rotation2d>> rv, double t0, double t1, double maxDx,
            double maxDy,

...
public class TimingUtil {
    ...
    public static <S extends State<S>, T extends State<T>> Trajectory<TimedState<S>, TimedState<T>> timeParameterizeTrajectory(
        ...
        for (int i = 0; i < states.size(); ++i) {
            ...
            while (true) {
                ...
                final double actual_acceleration = (constraint_state.max_translational_velocity
        ...
        for (int i = states.size() - 1; i >= 0; --i) {
            ...
            while (true) {
                ...
                final double new_max_translational_velocity = Math
                        .sqrt(successor.max_translational_velocity * successor.max_translational_velocity
        ...
        for (int i = 0; i < states.size(); ++i) {
            ...
            final double accel = (constrained_state.max_translational_velocity
                    * constrained_state.max_translational_velocity - v * v) / (2.0 * ds);
            double dt = 0.0;
            if (i > 0) {
            ...
            timed_headings
                    .add(new TimedState<>(constrained_state.heading, t, reverse ? -v : v, reverse ? -accel : accel)); // todo
                                                                                                                      // verify
        }
    ...
    protected static class ConstrainedState<S extends State<S>, T extends State<T>> {
        ...
        public String toString() {
            return state.toString() + ", distance: " + distance + ", max_translational_velocity: "
                    + max_translational_velocity + ", " +
                    "min_translational_acceleration: " + min_translational_acceleration + ", max_acceleration: "
                    + max_acceleration;
        }
    }
}
